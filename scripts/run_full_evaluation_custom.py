...
def evaluate_system_config(samples, cfg_name: str, cfg, args, device,
    ...
    if args.resume and checkpoint_path and checkpoint_path.exists():
        rows = read_jsonl(str(checkpoint_path), max_samples=None)
        if rows:
            print(f"  [{cfg_name}] resuming from checkpoint: {len(rows)}/{len(samples)} samples")
    ...
    for idx in range(start_idx, len(samples)):
        ...
        if checkpoint_path and (done % args.flush_every == 0 or done == len(samples)):
            save_jsonl(checkpoint_path, rows)
...
def maybe_load_planning_payload(out_dir: Path, dataset_name: str, cfg_name: str):
    agg_path = out_dir / f"{dataset_name}_{cfg_name}_aggregate.json"
    rows_path = out_dir / f"{dataset_name}_{cfg_name}_per_sample.jsonl"
    if not (agg_path.exists() and rows_path.exists()):
        return None

    payload = {
        "aggregate": read_json(agg_path),
        "rows": read_jsonl(str(rows_path), max_samples=None),
    }

    if dataset_name == "main":
        rob_path = out_dir / f"{dataset_name}_{cfg_name}_robustness.json"
        if rob_path.exists():
            rob = read_json(rob_path)
            payload["paraphrase_robustness"] = rob.get("paraphrase", {})
            payload["frame_noise_robustness"] = rob.get("frame_noise", {})

    return payload


def maybe_load_component_payload(out_dir: Path, component: str, dataset_name: str):
    agg_path = out_dir / f"{component}_{dataset_name}_aggregate.json"
    rows_path = out_dir / f"{component}_{dataset_name}_per_sample.jsonl"
    if not (agg_path.exists() and rows_path.exists()):
        return None, None
    return read_jsonl(str(rows_path), max_samples=None), read_json(agg_path)
...
def evaluate_planning_bundle(dataset_name: str, dataset_path: str, cfg, args, device, out_dir: Path, run_meta: Dict):
    ...
    for cfg_name in args.planning_configs:
        if args.resume:
            cached = maybe_load_planning_payload(out_dir, dataset_name, cfg_name)
            if cached is not None:
                print(f"\nEvaluating [{dataset_name}] {cfg_name} ...")
                print("  resume hit: using existing saved outputs")
                out["configs"][cfg_name] = cached
                continue

        print(f"\nEvaluating [{dataset_name}] {cfg_name} ...")
        progress_path = out_dir / f"{dataset_name}_{cfg_name}_progress.jsonl"
        rows = evaluate_system_config(samples, cfg_name, cfg, args, device, checkpoint_path=progress_path)
        payload = {
...
def main():
    ...
    parser.add_argument("--dataset_scope", choices=["all", "main", "cross"], default="all",
                        help="Run all datasets or only main/cross blocks")
    parser.add_argument("--resume", action="store_true",
                        help="Resume from existing saved outputs and per-config checkpoints")
    parser.add_argument("--flush_every", type=int, default=25,
                        help="Flush progress checkpoints every N samples")
    parser.add_argument(
        "--planning_configs",
    ...
    run_main = args.dataset_scope in ("all", "main")
    run_cross = args.dataset_scope in ("all", "cross") and (not args.skip_cross_eval)
    ...
    if args.skip_goal_eval:
        ...
        if run_main:
            if args.resume:
                cached_rows, cached_agg = maybe_load_component_payload(out_dir, "goal", "main")
                if cached_rows is not None:
                    goal_rows_main, goal_agg_main = cached_rows, cached_agg
                    print("Goal main: resume hit (using saved outputs)")
                else:
        ...
        if run_cross:
            if args.resume:
                cached_rows, cached_agg = maybe_load_component_payload(out_dir, "goal", "cross")
                if cached_rows is not None:
                    goal_rows_cross, goal_agg_cross = cached_rows, cached_agg
                    print("Goal cross: resume hit (using saved outputs)")
                else:
    ...
    if args.skip_critic_eval:
        ...
        if run_main:
            if args.resume:
                cached_rows, cached_agg = maybe_load_component_payload(out_dir, "critic", "main")
                if cached_rows is not None:
                    critic_rows_main, critic_agg_main = cached_rows, cached_agg
                    print("Critic main: resume hit (using saved outputs)")
                else:
        ...
        if run_cross:
            if args.resume:
                cached_rows, cached_agg = maybe_load_component_payload(out_dir, "critic", "cross")
                if cached_rows is not None:
                    critic_rows_cross, critic_agg_cross = cached_rows, cached_agg
                    print("Critic cross: resume hit (using saved outputs)")
                else:
