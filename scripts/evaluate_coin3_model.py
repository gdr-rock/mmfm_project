#!/usr/bin/env python3
"""Research-oriented evaluation for one System-1 model on coin_3.

Design choices:
  - Evaluate one System-1 checkpoint per run; compare checkpoints later with
    scripts/compare_coin3_evaluations.py.
  ...
  - Use prompt formats that exactly match coin_3 training prompts:
      * goal_only
...
def best_index_by_metric(candidates: list, key_name: str) -> int:
...
def oracle_index(candidates: list) -> int:
    valid = [cand for cand in candidates if cand["valid"]]
    if not valid:
        return 0
...
def select_candidate_index(config_name: str, candidates: list) -> int:
...
def case_json_record(case: EvalCase, candidates: list, selected_indices: dict[str, int]) -> dict:
    return {
        "case": asdict(case),
...
def aggregate_rows(rows: list, group_keys: list[str], *, bootstrap_iters: int, seed: int) -> list:
...
def build_markdown_table(rows: list, metrics: list[str], group_keys: list[str]) -> str:
    if not rows:
        return "| empty |\n|---|\n| no rows |\n"
...
def save_run_artifacts(
    ...
    *,
    ...
    case_records: list,
    flat_rows: list,
    summary_label_rows: list,
    summary_family_rows: list,
    per_task_rows: list,
) -> None:
    run_config = {
        "run_id": args.run_id,
...
def plot_metric_bars(summary_rows: list, out_dir: Path) -> None:
    try:
    ...
    for ax, metric_name in zip(axes, metric_names):
        for cfg_idx, config in enumerate(configs):
            ...
            ax.bar(pos, vals, width=width, label=format_config_label(config))
    ...
    fig.suptitle("Evaluation Metrics by Prompt Condition")
    fig.tight_layout()
    fig.savefig(out_dir / "plots" / "metrics_by_condition.png", dpi=180)
    plt.close(fig)


def plot_prefix_curves(summary_rows: list, out_dir: Path) -> None:
    try:
        import matplotlib.pyplot as plt
    except Exception:
        return
    ...
    for ax, metric_name in zip(axes, metric_names):
        for config in configs:
            ...
            if xs:
                ax.plot(xs, ys, marker="o", label=format_config_label(config))
...
def plot_accuracy_by_gold_len(flat_rows: list, out_dir: Path) -> None:
    try:
        import matplotlib.pyplot as plt
    except Exception:
        return
...
def plot_reranking_gains(summary_family_rows: list, out_dir: Path) -> None:
    try:
        import matplotlib.pyplot as plt
    except Exception:
        return
...
def plot_task_heatmaps(per_task_rows: list, out_dir: Path) -> None:
    try:
        import matplotlib.pyplot as plt
    except Exception:
        return
...
def make_plots(out_dir: Path, flat_rows: list, summary_label_rows: list, summary_family_rows: list, per_task_rows: list) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate one System-1 model on coin_3 with clean reranking ablations")
    ...
    parser.add_argument("--goal_only_data", default="data/coin_3/coin3_system1_test_goal_only.jsonl")
    parser.add_argument("--goal_interp_data", default="data/coin_3/coin3_system1_test_goal_plus_interpretation.jsonl")
    ...
    parser.add_argument("--output_dir", default="outputs/coin_3")
    ...
    args.run_id = args.run_id or f"{args.model_tag}_coin3_eval"
    ...
    if checkpoint_per_case.exists() and checkpoint_per_sample.exists():
        ...
        evaluated_case_ids = {r["case"]["case_id"] for r in case_records}
