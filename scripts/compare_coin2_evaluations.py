#!/usr/bin/env python3
"""Compare multiple coin_2 evaluation runs.

This script expects each run directory to be produced by
`scripts/evaluate_coin2_model.py` and to contain at least:
  - run_config.json
  - summary_overall.csv
  - summary_family.csv
...
def find_run_dirs(args) -> list[Path]:
    if args.run_dirs:
...
def load_runs(run_dirs: list[Path], scope: str) -> list[dict]:
    runs = []
    for run_dir in run_dirs:
        ...
        runs.append(
            {
                "run_dir": run_dir,
                "run_label": run_cfg.get("model_tag", run_dir.name),
...
def build_combined_rows(runs: list[dict], configs: set[str] | None) -> list[dict]:
    out = []
    for run in runs:
...
def write_markdown(path: Path, rows: list[dict], headers: list[str]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    if not rows:
...
def comparison_table_rows(rows: list[dict], scope: str) -> list[dict]:
...
def best_rows(rows: list[dict], scope: str) -> list[dict]:
    key_name = "condition_label" if scope == "label" else "condition_family"
...
def interpretation_gain_rows(runs: list[dict]) -> list[dict]:
    out = []
    for run in runs:
        by_cfg_family = {
...
def plot_metric_grids(rows: list[dict], out_dir: Path, scope: str) -> None:
    try:
...
def plot_prefix_curves(rows: list[dict], out_dir: Path) -> None:
    try:
        import matplotlib.pyplot as plt
    except Exception:
        return
...
def plot_interpretation_gain(gain_rows: list[dict], out_dir: Path) -> None:
    try:
        import matplotlib.pyplot as plt
    except Exception:
        return
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare multiple coin_2 evaluation runs")
    parser.add_argument("--input_root", default="outputs/coin_2")
    ...
    parser.add_argument("--output_dir", default="outputs/coin_2/comparison")
