...
try:
    with open("data/coin_2/coin_interpretations_verified.jsonl", "r") as f:
        for line in f:
            data = json.loads(line)
            CUSTOM_INTERPRETATIONS[data["video_id"]] = data["interpretation"]
    print(f"Loaded {len(CUSTOM_INTERPRETATIONS)} custom interpretations.")
except Exception as e:
    print("Could not load custom interpretations:", e)
    ...
    for key, record in sorted(task_records.items(), key=lambda item: (item[0][0], int(item[0][1]))):
        ...
        for video_id, video in sorted(record["videos"].items()):
            ...
            for prefix_len in distinct_prefix_lengths(valid_prefixes, samples_per_video, rng):
                ...
                # Use custom video interpretation if available
                vid_interp = CUSTOM_INTERPRETATIONS.get(video_id, record["interpretation"])
                variants = [
                    ("goal_only", ""),
                    ("goal_plus_interpretation", vid_interp),
                ]
...
def build_full_plan_rows(
    ...
    *,
    ...
    for key, record in sorted(task_records.items(), key=lambda item: (item[0][0], int(item[0][1]))):
        ...
        # We now loop over videos to emit per-video test items with their unique custom interpretation
        # if a custom interpretation exists, otherwise we fallback.
        # We will emit video-level prompts for the full plan.
        for video_id, video in sorted(record.get("videos", {}).items()) if record.get("videos") else [("no_video", {})]:
            vid_interp = CUSTOM_INTERPRETATIONS.get(video_id, record.get("interpretation", ""))
            ...
            for prompt_variant, interpretation in variants:
                    row = {
                        ...
                        "meta": {
                            ...
                            "video_id": video_id,
                            "split": split,
                            "prompt_style": "full_plan_from_task_with_video_context" if video_id != "no_video" else "full_plan_from_task",
                            "prompt_variant": prompt_variant,
                            "prefix_len": 0,
...
def main() -> None:
    ...
    parser.add_argument("--crosstask_tasks_file", default="data/crosstask/release/crosstask_release/tasks_primary.txt")
    ...
    write_jsonl(out_dir / "coin3_system1_train.jsonl", train_rows)
    write_jsonl(out_dir / "coin3_system1_val.jsonl", val_rows)
    write_jsonl(out_dir / "coin3_system1_test_prompt_variants.jsonl", test_rows)
    write_jsonl(out_dir / "coin3_system1_test_goal_only.jsonl", test_goal_only)
    write_jsonl(
        out_dir / "coin3_system1_test_goal_plus_interpretation.jsonl",
        test_goal_plus_interpretation,
    )

    manifest_rows = build_split_manifest(task_records, split_map)
    write_manifest_csv(out_dir / "coin3_task_split_manifest.csv", manifest_rows)

    report = compute_report(task_records, split_map, train_rows, val_rows, test_rows)
    (out_dir / "coin3_dataset_report.json").write_text(
    ...
    print(f"manifest: {out_dir / 'coin3_task_split_manifest.csv'}")
    print(f"report:   {out_dir / 'coin3_dataset_report.json'}")
